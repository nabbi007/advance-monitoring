...
# ECR outputs (Terraform creates the repos automatically)
ECR_REGISTRY=$(terraform output -raw ecr_registry 2>/dev/null || echo "")
ECR_BACKEND_REPO=$(terraform output -raw ecr_backend_repository_url 2>/dev/null || echo "")
ECR_FRONTEND_REPO=$(terraform output -raw ecr_frontend_repository_url 2>/dev/null || echo "")
AWS_REGION=$(terraform output -raw aws_region 2>/dev/null || grep aws_region terraform.tfvars | awk -F'"' '{print $2}' || echo "us-east-1")

# SSH key — auto-generated by Terraform; extract from state and write to disk.
SSH_KEY="${SSH_KEY_PATH:-$HOME/.ssh/deploy_key.pem}"

if [[ ! -f "$SSH_KEY" ]]; then
  log "Writing SSH private key from Terraform output → $SSH_KEY"
...
log "ECR Backend:            $ECR_BACKEND_REPO"
log "ECR Frontend:           $ECR_FRONTEND_REPO"
log "SSH Key:                $SSH_KEY"
...
# ECR configuration (auto-populated from Terraform outputs)
ecr_registry=${ECR_REGISTRY}
ecr_backend_repo=${ECR_BACKEND_REPO}
ecr_frontend_repo=${ECR_FRONTEND_REPO}
aws_region=${AWS_REGION}

# Docker images — use ECR repos with :latest tag
app_backend_image=${ECR_BACKEND_REPO}:latest
app_frontend_image=${ECR_FRONTEND_REPO}:latest
EOF
